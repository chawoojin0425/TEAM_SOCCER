...
# ... (기존 process_detection_and_classification 함수) ...
...
class App(ctk.CTk):
    ...
    def update_player_list(self, players_to_draw):
        ...
        # 새 항목(프레임 + 라벨 + 버튼) 생성
        for player in players_to_draw:
            # 🚨 수정된 부분: 팀 약자 없이 선수 이름만 사용
            player_info = f"{player['name']}"
            player_id = player.get('id', 'UNKNOWN') # 고유 ID는 유지
            ...
            player_label = ctk.CTkLabel(item_frame,
                                        text=player_info, # <--- 수정된 player_info 사용
                                        anchor="w",
                                        padx=5,
                                        pady=2)
